...
#el codi original era realment un "shift_columns", com esta a la funcio "inv_shift_rows", que no hem tocat
def shift_rows(s):
...
class AES(object):
    ...
    def encrypt_block(self, plaintext):
        ...
        #el codi original només feia una iteració en aquest loop. Per tant, no feia les 9+1 iteracions
        #necessaries segons AES-128, sino que feia 1+1.
        for i in range(0, self.n_rounds-1, 1):
            sub_bytes(plain_state)
            shift_rows(plain_state)
            mix_columns(plain_state)
            add_round_key(plain_state, self._key_matrices[i])
            print("iteracio: " + str(i+1))
            escriuBloc(-1,matrix2bytes(plain_state))

        print("iteracio: " + str(i+2))
        escriuBloc(-1,matrix2bytes(plain_state))
        sub_bytes(plain_state)
        shift_rows(plain_state)
    ...
    def encrypt_cbc(self, plaintext, iv):
        ...
        for i in range(0, len(plaintext), 16):
            ...
            print("bloc a encriptar numero: " + str(i//16+1))
...
#sembla que encripta un minim de 2 blocs de 128 bits de text. Per tant, si el text es <= 256 bits,
#s'omplira fins arribar a 2 blocs de 128 bits cadascun
...
def main():
    listBloc= [[7, 160, 51, 115], [55, 167, 50, 43], [125, 16, 28, 225], [141, 20, 69, 161]]
    #07 a0 33 73
    #37 a7 32 2b
    #7d 10 1c e1
    #8d 14 45 a1
